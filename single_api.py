#!/usr/bin/env python3
"""
FastAPI server for ASR → LLM → TTS (XTTS) streaming with gapless audio chunking and cancellation.
"""

import os, re, time, threading, math, asyncio, json, uuid
...
def to_8k(x: np.ndarray, src_sr: int) -> np.ndarray:
    """Resample arbitrary SR → 8 kHz (mono, float32 in [-1,1])."""
    return resample_poly(x, 8_000, src_sr).astype(np.float32)

def audio_to_base64(audio_data: np.ndarray) -> str:
    """Convert audio array to base64 for web transmission (int16)."""
    import base64
...
class AudioSession:
    ...
    async def process_utterance(self, wav16k: np.ndarray):
        ...
        try:
            ...
            self.last_tts_response = (response, lang, response_id)
            self.tts_cancel_event.clear()
            tts_thread = threading.Thread(
                target=self._tts_stream_worker,
                args=(response, lang, response_id)
            )
    ...
    def _tts_stream_worker(self, response, lang, response_id):
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        try:
            coro = self._tts_stream(response, lang, response_id)
            loop.run_until_complete(coro)
        finally:
            loop.close()

    async def _tts_stream(self, response, lang, response_id):
        try:
            ...
            for i, chunk in enumerate(xtts_model.inference_stream(
                ...
                # Post-process chunk and send
                chunk_np = chunk.clone().detach().cpu().numpy()
                chunk_np = np.clip(chunk_np, -1, 1)
                chunk8k = resample_poly(chunk_np, 8000, 24000).astype(np.float32)
